...
void MainWindow::on_pushButton_3Transaction_clicked()
{
    ...
    // Display search criteria in textBrowser_transaction
    QString searchResults ;

    
    // Determine search criteria type
    bool isNumber = false;
    searchText.toInt(&isNumber);
    
    if (isNumber) {
        searchResults += "Critère: Référence - " + searchText + "\n\n";
    } else {
        // Check if it matches transaction types
        QString upperSearch = searchText.toUpper();
        if (upperSearch == "ACHAT" || upperSearch == "ACHATS" || 
            upperSearch == "VENTE" || upperSearch == "VENTES") {
            searchResults += "Critère: Type de transaction - " + searchText + "\n\n";
        } else {
            // Query to check if it's a client or partenaire
            QSqlQuery checkQuery;
            checkQuery.prepare("SELECT 'CLIENT' as TYPE FROM SYRINE.CLIENT WHERE UPPER(NOM) LIKE UPPER(:search) "
                             "UNION "
                             "SELECT 'PARTENAIRE' as TYPE FROM SYRINE.PARTENAIRE WHERE UPPER(NOMENTREPRISE) LIKE UPPER(:search)");
            checkQuery.bindValue(":search", "%" + searchText + "%");
            
            if (checkQuery.exec() && checkQuery.next()) {
                QString type = checkQuery.value(0).toString();
                if (type == "CLIENT") {
                    searchResults += "Critère: Nom du client - " + searchText + "\n\n";
                } else {
                    searchResults += "Critère: Nom du partenaire - " + searchText + "\n\n";
                }
            } else {
                searchResults += "Critère: Terme général - " + searchText + "\n\n";
            }
        }
    }
...
{
    // Launch the chart statistics dialog
...
void MainWindow::showTransactionCharts()
{
    ...
    chartDialog->setAttribute(Qt::WA_DeleteOnClose, true); // Ensure dialog is deleted when closed

    QHBoxLayout *mainLayout = new QHBoxLayout(chartDialog);

    // Bar Chart - Payeurs/Clients/Partenaires with montant intervals
    QChart *barChart = new QChart();
    barChart->setTitle("Distribution des montants par type de payeur");
    ...
    *interval50000plus << clientCounts[4] << partenaireCounts[4];
    ...
    pieChart->setTitle("Distribution des achats et ventes");
